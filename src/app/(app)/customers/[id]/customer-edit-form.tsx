      ...
      <form action={formAction} className="space-y-5" noValidate>
          ...
          <fieldset className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => {
                ...
                return (
                  <label
                    className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm font-medium text-slate-700 has-[:checked]:border-[var(--primary)] has-[:checked]:bg-[var(--primary-soft)]"
                    key={tag.id}
                  >
                    <input
        ...
        <div className="flex flex-col gap-2 border-t border-[var(--border)] pt-5 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-slate-500">
            변경 내용은 고객 상세와 연결된 운영 화면에 바로 반영됩니다.
          </p>
